...
ggplot(logdf, aes(x = Fict1Fact2, y = response, fill = Fict1Fact2)) + theme_minimal() + labs(x = "Fiction = 0 & fact = 1", y = "Mean story rating") + geom_bar(aes(fill=Fict1Fact2), stat='summary', fun.y = mean, width = 0.3) + stat_summary(fun.data = mean_se, geom = "errorbar", color = 'black', width = 0.1) + ggtitle("Mean story rating with errorbars")
...
hrmodel1 <- lmerTest::lmer(value ~ abs(sentence_sen_mean) + drift + (1|Participant_number), data = pivotdf)
summary(hrmodel1)

hrmodel2 <- lmerTest::lmer(value ~ sentence_sen_mean + drift + (1|Participant_number), data = pivotdf)
summary(hrmodel2)
...
hrmodel7 <- lmerTest::lmer(value ~ fictfactsentence_sen_mean + drift + (1|Participant_number) + (1|group), data = pivotfictfactdf)
