...
sentimentassumptions_qq <- ggplot(logdf, aes(sample = response)) + stat_qq()+ stat_qq_line(colour = "red")
sentimentassumptions_qq

responseqq <- ggplot(logdf, aes(sample = response)) + stat_qq()+ stat_qq_line(colour = "red")
responseqq

round(pastecs::stat.desc(logdf$response, basic = FALSE, norm = TRUE), digits = 2)
barplot(logdf_fictfact$`mean(response)`)
#Statdesc
sentimentassumptions_stat <- round(pastecs::stat.desc(logdf$Sentida_mean, basic = FALSE, norm = TRUE), digits = 2)
...
ttt <- t.test(response ~ Fict1Fact2, data = logdf)
...
ggplot(logdf_fictfact, aes(x = story_num, y = `mean(response)`, col = Fict1Fact2, col = Fict1Fact2, shape = Fict1Fact2)) + geom_point() + labs(x="response", y="Sentida_mean")

ggplot(logdf_fictfact, aes(x = story_num, y = `mean(response)`, col = Fict1Fact2, shape = Fict1Fact2)) + geom_point() + labs(x="Story number", y="Mean response") + ggtitle("Mean response for each story")
...
#After filter P1
ggplot(test2, aes(x = t, y = V2)) + geom_path() + geom_path(aes(y = V3 + m), col = "red")
...
# for(col in 1:ncol(newdf1)) { # for-loop over columns
#   for(row in 2:(nrow(newdf1)-1)){  #for-loop over rows
#     value = newdf1[row,col]
#     if(value > 3){
#     newdf1[row,col] <- 3
#     }
#     if(value < -3){
...
reshrm1 <- resid(hrmodel1)
plot(hrmodel1)
qqnorm(reshrm1)
pivotdf$abs_sent <- abs(pivotdf$sentence_sen_mean)
ggplot(pivotdf, aes(sample = abs_sent)) + stat_qq()+ stat_qq_line(colour = "red")
...
reshrm3 <- resid(hrmodel3)
plot(hrmodel3)
qqnorm(reshrm3)
ggplot(pivotdf, aes(sample = sentence_sen_mean)) + stat_qq()+ stat_qq_line(colour = "red")

ggplot(logdf, aes(sample = Sentida_mean)) + stat_qq()+ stat_qq_line(colour = "red")
...
MuMIn::r.squaredGLMM(hrmodel3pushed)
...
MuMIn::r.squaredGLMM(hrmodel6_new)
resm6 <- resid(hrmodel6_new)
plot(hrmodel6_new)
qqnorm(resm6)
assumptHRV
value_fict_qq
...
assumptHRV <- round(pastecs::stat.desc(pivotfictfactdf$value, basic = FALSE, norm = FALSE), digits = 2)
#HR plots
value_fict_qq <- ggplot(pivotfictfactdf, aes(sample = value)) + stat_qq()+ stat_qq_line(colour = "red")
...
MuMIn::r.squaredGLMM(hrmodel6_new1)
...
abs_sentence <- abs(sentence_sen_df$sentence_sen_df)
abs_sentence <- as.data.frame(abs_sentence)
...
pivotfactpushed$group <- 1
pivotfictpushed$group <- 2
...
pivotfictfactdfpushed$group <- as.factor(pivotfictfactdfpushed$group)
pivotfictfactdfpushed$fictfact <- as.numeric(pivotfictfactdfpushed$fictfact)
str(pivotfictfactdfpushed)
...
hrmodel6pushed <- lmerTest::lmer(value ~ fictfact*group + (1|Participant_number), data = pivotfictfactdfpushed)
...
hrmodelsen1 <- lmerTest::lmer(value ~ sentence_sen_mean + (1|Participant_number), data = pivotfictfactdfpushed)

summary(hrmodelsen1)

MuMIn::r.squaredGLMM(hrmodelsen)

as.data.frame(hrdata1)
hrdata1$meanhr <- rowMeans(hrdata1)
...
newdf1 <- as.matrix(newdf1)
ahh <-filter(but,newdf1)
ahh <- as.data.frame(ahh)
...
plot(spy[1:1000]~spx[1:1000],xlab="frequency (Hz)",ylab="spectral density",type="l",col='darkgreen', main = "Spectral elements of mean HRV after filtering")
?plot
...
subset_meanHR_sentencemean <- scaledhrdata1 %>% select(time, meanHR, abs_sent)
...
p <- g1 <- subset_meanHR_sentencemean %>% dygraph() %>% dyOptions(stackedGraph = TRUE) %>%
...
p1 <- dygraph(subset_meanHR_sentencemean)
p1

pushedhrdata1$abs_sent <- scaledhrdata1$abs_sent
...
str(pushedhrdata1)
#Abs
subset_pushedmeanHR_sentencemean <- pushedhrdata1 %>% select(time, hrmean, fictfact)
subset_pushedmeanHR_sentencemean$normal<-fftfilt(rep(1, 10)/10, subset_pushedmeanHR_sentencemean$hrmean)
subset_pushedmeanHR_sentencemean$hrmean=NULL

g1 <- subset_pushedmeanHR_sentencemean %>% dygraph() %>% dyOptions(stackedGraph = TRUE) %>%
  dyRangeSelector(height = 20)
